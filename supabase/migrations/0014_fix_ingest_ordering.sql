-- 0014 — تصحيح ترتيب الاحتساب داخل eo_ingest_activity
--
-- الخطأ الذي عُولج هنا:
--   كان تحديث "آخر تفاعل" يسبق ترحيل دقائق النشاط، فإذا عاد الموظف بعد خمول
--   طويل ثم تفاعل، كان مدى الترحيل يمتد من آخر لحظة محسوبة حتى الآن — أي أن
--   نقرة واحدة بعد ٢٥ دقيقة خمول كانت تمنحه ٢٥ دقيقة نشاطًا.
--
-- الترتيب الصحيح المطبَّق الآن:
--   ١) رحّل النشاط أولًا باستخدام الحالة القديمة ⇒ يُحتسب حتى نهاية نافذة
--      التفاعل السابق فقط (آخر تفاعل + عتبة الخمول).
--   ٢) ثم سجّل التفاعل الجديد وادفع marked_until إلى الآن ⇒ فجوة الخمول
--      بين النافذتين لا تُحتسب أبدًا.
  ...
  if not found then
    ...
    update emp_ops.employee_runtime_state
       set attendance_session_id = v_session.id,
           ...
           marked_until        = case when v_interactive
                                      then greatest(coalesce(marked_until, now()), now())
                                      else marked_until end,
           updated_at          = now()
