...
| `app_settings` | كل رقم تشغيلي في النظام: عتبة الخمول، فترة النبض، الإغلاق التلقائي… مع `min_value`/`max_value` تُفحص على الخادم، و`is_public` لتحديد ما يراه الموظف العادي. |
...
| `activity_heartbeats` | النبضات مجمَّعة في نوافذ زمنية على الخادم. يخزّن `clock_skew_seconds` كمؤشر تدقيق على العبث بساعة الجهاز. |
